...
class PropositionsQuizz extends Component {
   ...
   renderReponse() {
    ...
    const randomlySortedAnswers = [correctAnswer, ...incorrectAnswers]
        ...
        .map((answer, i) => (
            <div key={answer}>
                <input className="champReponse" type="radio" name="name" id={`propo-${i}`}/>
                <label onClick={this.hide} htmlFor={`propo-${i}`} className='proposition'>{answer}</label>
            </div>
        ));
    ...
    render() {
        ...
        return (
            <div>
                {isShowing && <div onClick={this.closeModalHandler} className="back-drop"></div>}
                {this.renderReponse()}
                <div>
                    <button 
